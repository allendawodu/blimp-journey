...
func _on_state_physics_processing(delta: float) -> void:
	var direction_to_player: Vector2 = (_player.global_position - _target.global_position).normalized()
	_target.velocity = _target.velocity.lerp(direction_to_player * _target.max_speed, 0.1)
	...
	if distance_to_player < 100:
		if _player_visibility.is_player_visible:
			...
			if _level and _level.scene.scene_name == "roof":
				_player.get_node("HealthComponent").damage(10)
				_chart.send_event("wait")
			else:
				Dialogic.signal_event.emit("death_by_flying_ghost")
