...
## Custom method to start dialogue from anywhere.
func quick_start(timeline: Variant, label: String = "", characters: Array[TextBubbleMarker] = []) -> void:
	var layout: Node = start(timeline, label)
	for marker: TextBubbleMarker in characters:
		layout.register_character(marker.character, marker)
