...
@onready var choice_container: Container = (%ChoiceContainer as HFlowContainer)
...
func _process(delta:float) -> void:
	...
	var point_b: Vector2 = (base_position - position)
	...
	tail.width = minf(bubble_rect.size.x / 2.0, base_tail_width)
...
func add_choice_container(node:Container, alignment:=FlowContainer.ALIGNMENT_BEGIN) -> void:
	choice_container.get_children().map(func(child): child.queue_free())
