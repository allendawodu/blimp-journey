...
## Edit this to change the default text color.
## It's mixed with the modulate property of the text bubble.
@export var default_text_color := Color("#222222")
...
func reveal_text(_text: String, keep_previous:=false) -> void:
	...
	if !keep_previous:
		...
		text = '[color=' + default_text_color.to_html(false) + ']' + text
