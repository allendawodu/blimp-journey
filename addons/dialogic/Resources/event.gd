...
#region HELPERS
func _enum_to_suggestions(keys: Array, values: Array) -> Dictionary:
	...
	for i: int in keys.size():
		...
		result[key.to_pascal_case()] = {"value": value, "text_alt": key.to_lower()}

	return result


func _enum_to_options(keys: Array, values: Array) -> Array[Dictionary]:
