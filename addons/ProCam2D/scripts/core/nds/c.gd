...
func check_target_reached(camera) -> void:
	var velocity_length: float = camera._current_velocity.length()
	var zoom_velocity: float = absf(camera._current_zoom_velocity)
	var rotation_velocity: float = absf(camera._current_rotation_velocity)
	
	if velocity_length < 10 and zoom_velocity < 0.01 and rotation_velocity < 0.01:
...
func _on_hold_time_complete() -> void:
	...
	EventBus.camera_event_ended.emit()
