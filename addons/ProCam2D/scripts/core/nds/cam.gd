...
func _ready() -> void:
	...
	_setup_spatial_hash()
	_setup_camera()
	EventBus.camera_event_started.connect(set_cinematic)
...
func _setup_camera() -> void:
	# NOTE: Disabled because it was messing up scene switching
	...
	_camera.process_callback = Camera2D.CAMERA2D_PROCESS_PHYSICS if get_tree().physics_interpolation else Camera2D.CAMERA2D_PROCESS_IDLE
...
func _apply_cinematic_influences(pos: Vector2) -> Vector2:
	...
	current_cinematic.check_target_reached(self)
...
## Start a new cinematic or take advantage of the existing one.
## [param cinematic_id] The id of the cinematic to start.
## [param index] The index of the cinematic to start, -1 to go to the next cinematic.
func set_cinematic(cinematic_id: Variant, index: int = 0) -> void:
	if _active_cinematic_id == cinematic_id:
		if index == _current_cinematic_index:
			return
		elif index == -1:
			_active_cinematics[_current_cinematic_index].is_complete()
		elif index >= 0 and index < _active_cinematics.size():
			_active_cinematics[_current_cinematic_index].stop()
			_current_cinematic_index = index
			_active_cinematics[_current_cinematic_index].start(self)
	else:
		stop_cinematic()
		start_cinematic(cinematic_id, index)

func start_cinematic(cinematic_id, index: int = 0) -> void:
	...
	if index > _active_cinematics.size() - 1:
		push_error("Index out of range for cinematic: ", id_string)
		return
	_active_cinematics[index].start(self)
