...
static func dict2fields(dict : Dictionary) -> Dictionary:
	...
	for field in dict.keys():
		...
		match typeof(field_value):
			...
			_:
				# TODO: Add all the convertible types (i.e., everything except TYPE_CALLABLE and TYPE_SIGNAL)
		...
		elif var_type == "stringValue":
			fields[field] = to_firebase_type(field_value)
		else:
...
class FirebaseTypeConverter extends RefCounted:
	...
	func _to_color(value):
		return str_to_var(value) as Color
...
static func from_firebase_type(value):
	...
	if value.has("mapValue"):
		...
		if type == "stringValue":
			...
			if split_type in [ "Vector2", "Vector2i", "Color" ]:
...
static func to_firebase_type(value : Variant) -> Dictionary:
	...
	match typeof(value):
		...
		TYPE_VECTOR2, TYPE_VECTOR2I, TYPE_COLOR:
			var_type = "stringValue"
...
static func array2fields(array : Array) -> Array:
	...
	for field in array:
		match typeof(field):
			...
			TYPE_VECTOR2, TYPE_VECTOR2I, TYPE_COLOR:
				var_type = "stringValue"
				fields.append(to_firebase_type(field))
				continue
