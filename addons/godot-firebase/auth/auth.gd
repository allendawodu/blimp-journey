...
const RESPONSE_LINK_ACCOUNT : String = "identitytoolkit#SetAccountInfoResponse"
const RESPONSE_EMAIL_VERIFICATION : String = "identitytoolkit#GetOobConfirmationCodeResponse"
...
func _on_FirebaseAuth_request_completed(result : int, response_code : int, headers : PackedStringArray, body : PackedByteArray) -> void:
	...
	if response_code == HTTPClient.RESPONSE_OK:
		if not res.has("kind"):
			...
			match res.kind:
				RESPONSE_SIGNUP, RESPONSE_LINK_ACCOUNT:
					auth = get_clean_keys(res)
				...
				RESPONSE_EMAIL_VERIFICATION:
					auth_request.emit(1, res)
				_:
					Firebase._printerr("Unknown response kind: %s" % res.kind)
